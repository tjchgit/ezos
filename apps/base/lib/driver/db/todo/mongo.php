<?php
class base_db_mongo extends db{
    ...
    public function switchCollection($collection,$db='',$master=true){
        ...
        try{
            ...
            kernel::throwException($e->getMessage());
        }
    }
    ...
    public function command($command=array()) {
        ...
        $this->debug();
        if(!$result['ok']) {
            kernel::throwException($result['errmsg']);
        }
        return $result;
    }
        ...
        foreach ($where as $key=>$val){
            if('_id' != $key && 0===strpos($key,'_')) {
                ...
                if(!preg_match('/^[A-Z_\|\&\-.a-z0-9]+$/',trim($key))){
                    kernel::throwException(L('_ERROR_QUERY_').':'.$key);
                }
                $key = trim($key);
                if(strpos($key,'|')) {
                    $array   =  explode('|',$key);
