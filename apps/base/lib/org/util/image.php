...
class image {
     ...
     * @return void
     */
     static public function water($source, $water, $savename=null, $waterPos=0, $alpha=80) {
        //  如果图片或者水印不存在,直接返回
        ...
        $pos    = self::position($sInfo, $wInfo, $waterPos);
        //如果图片小于水印图片，不生成图片
        if(!$pos){
            return false;
        }
        ...
        $groundImage = self::copyImage($sImage, $wImage, $pos, $wInfo);
    ...
    static function copyImage($groundImg, $waterImg, $pos, $waterInfo){
        imagealphablending($wImage, true);
    ...
    static function position($groundInfo, $waterInfo, $waterPos){
